...
public class TodoController {
  ...
  public ResponseEntity<?> testTodo() {
    ...
    return ResponseEntity.ok().body(response); //ResponseEntity의 바디는 모두 JSON형태로 넘어감
  }

  //로그인 후 App.js에서 Get으로 요청하게 되므로, 로그인한 유저의 게시물 정보를 서비스를 이용하여 리턴할게요.
  @GetMapping
  ...
  public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto,
    ...
    try {
      ...
      // (3) 임시 유저 아이디를 설정 해 준다. 이 부분은  인증과 인가에서 수정 할 예정이다. 
  ...
  public ResponseEntity<?> deleteTodo(@RequestBody TodoDTO dto, @AuthenticationPrincipal String userId) {
    try {
      ...
      // (2) 임시 유저 아이디를 설정 해 준다. 지금은 인증과 인가 기능이 없으므로 한 유저(temporary-user)만 로그인 없이 사용 가능한 애플리케이션인 셈이다
      entity.setUserId(userId);
      ...
      // (5) 변환된 TodoDTO리스트를 이용해ResponseDTO를 초기화한다.
      ResponseDTO<TodoDTO> response = ResponseDTO.<TodoDTO>builder().data(dtos).build();
      System.out.println("삭제완료: "+response);
