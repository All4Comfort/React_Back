...
public class TodoService {
  ...
  @Autowired
  private TodoRepository repository; //DB에 CRUD 하는 역할 : select, find, delete

  public String testService() {
    // TodoEntity 생성 : Entity는 곧 table
    TodoEntity entity = TodoEntity.builder().title("My first todo item").build();
    // TodoEntity 저장
    repository.save(entity); //Insert or (기존 데이터가 존재하면 바뀐 내용만 조회하여) Update 처리
    // TodoEntity 검색
    TodoEntity savedEntity = repository.findById(entity.getId()).get(); //id값으로 엔티티 찾기
    return savedEntity.getTitle(); //찾은 엔티티의 title값 리턴
  }
